...
-- 1. CUSTOMERS
-- ==============================
PRINT 'Step 1: Inserting customers...';
SET @i = 1;
WHILE @i <= 150
BEGIN
...
-- 2. ADDRESSES
-- ==============================
PRINT 'Step 2: Inserting addresses...';
...
-- ==============================
-- 3. ORDERS (skip if already seeded)
-- ==============================
...
BEGIN
    ...
    SELECT TOP 2000 Id, Price, Name, ISNULL(SKU,'') FROM Products ORDER BY NEWID();
    ...
    PRINT CONCAT('Product pool: ', @pCnt, ', Customer pool: ', @cCnt);

    PRINT 'Step 4: Inserting orders...';
    DECLARE @n   INT = 1;
    ...
    BEGIN
        ...
        BEGIN
            DECLARE @prog INT = @n;
            PRINT CONCAT('  Progress: ', @prog, '/8000');
        END;
    END;
